...
//#else
//    {
//        float lux_real = 0.0f;                /* lux_real（本次真实读取到的光照值） */
//        static uint8_t bh1750_fail_cnt = 0U; /* bh1750_fail_cnt（BH1750连续失败计数） */

//        if (BSP_BH1750_ReadLux(&lux_real) == HAL_OK)
//        {
//            /* 读取成功：更新光照值，并清零失败计数 */
//            data->light_lux = lux_real;
//            bh1750_fail_cnt = 0U;
//        }
//        else
//        {
//            /* 读取失败：先累计失败次数 */
//            if (bh1750_fail_cnt < 255U)
//            {
//                bh1750_fail_cnt++;
//            }

//            /* 连续失败 3 次后，重新初始化 BH1750 */
//            if (bh1750_fail_cnt == 3U)
//            {
//                BSP_BH1750_Init();
//            }

//            /* 连续失败很多次，说明当前不是正常光照值，而是传感器掉线 */
//            if (bh1750_fail_cnt >= 6U)
//            {
//                data->light_lux = -1.0f;   /* -1.0f（无效值）用于提示当前 BH1750 异常 */
//            }
//        }
//    }
//#endif
...
void APP_Sensor_Init(SensorData_t *data)
{
    ...
    data->air_humi      = 60.0f;
    data->light_lux     = 300.0f;
    data->soil_moisture = 2500.0f;  /* 这里先给一个 ADC 原始值风格的初值 */
    data->ph_value      = 6.5f;
}
...
void APP_Sensor_Update(SensorData_t *data)
{
...
#else
    {
        ...
        else
        {
            ...
            /* 连续失败很多次，说明当前 BH1750 异常 */
            if (bh1750_fail_cnt >= 6U)
            {
                data->light_lux = -1.0f;   /* -1.0f 表示无效值 */
            }
            /* 否则保留上一次有效值 */
        }
    }
